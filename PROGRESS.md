...
| `9aa1c15` | Operations board · dual sign-off · eBay order split · finance reconciliation |
| `663bb32` | Banner position grid · card list editor · text-selection fix in the block editor |
| `c00759b` | PROGRESS.md brought current for sessions 76–79 |
...
| Item | Priority | Notes |
|---|---|---|
| **Orders list has no `sort` parameter** | Medium — backend | `AdminOrderController` is `orderByDesc('created_at')`, hardcoded. The fulfilment queue therefore shows **newest first**, which is the wrong end for a queue: the row that has waited longest is the one somebody is chasing. **Deliberately not fixed client-side** — sorting the 25 rows already fetched and calling the result "oldest" would leave the genuinely oldest orders on page two while the UI claimed otherwise, which is a worse lie than the one it replaces. Caught in session 87 as a false sentence already written into the UI: the copy said "oldest first" and the `sort: "oldest"` parameter was being silently ignored |
| **`you_may_sign` is missing from the embedded sign-off block** | Medium — backend | `GET /admin/orders/{id}` embeds `signoff` via the shared `state()`, which does **not** add `you_may_sign` — that is added only in `AdminOrderSignoffController:39`. So the note's "the order page needs no second request" and "drive the button off `you_may_sign`" cannot both hold. It cannot be derived client-side either: `canSign()` compares `admin_user_id` for the two-different-people rule, and the slots carry a display *name*. The panel currently paints from the embedded block and makes one extra request purely to learn which button to offer. **Adding `you_may_sign` to `state()` removes that request entirely** |
| **Order list rows carry no document state** | Medium — backend | `formatOrderList()` returns no `trade_documents` field; documents are on the *detail* payload only. So the fulfilment queue cannot show the "documents sent?" column that was asked for without one request per row. The queue says plainly that it doesn't know, rather than asserting it. **One field on the list row** — a count, or `last_document_sent_at` — would make the column real |
| **`orders.view` diverges from the backend, in both directions** | Medium | Backend grants it to `super_admin, admin, order_manager, sales_manager, **finance**`; this repo has `… **support**`. `finance` was added in session 83 because sign-off depends on it. **`support` was deliberately left**: removing it takes Orders away from a role that has it in the UI today, which is a product decision rather than a typo — and it is already a live divergence, since support is shown orders the server would 403. Same shape as the `analytics.view` row below; **worth settling both in one pass** rather than a third time |
| **`behaviour-analytics.tsx` charts have the recharts mount-animation defect** | Low | Recharts draws a line in by animating `stroke-dasharray` from zero, so the chart is empty until rAF runs — which means a chart in a **background tab renders blank until focused**, and any screenshot catches it mid-draw. Fixed in `operations-report.tsx` with `isAnimationActive={false}`; the behaviour-analytics charts still have it. One line, and it compounds the row below — the page has never been looked at with data in it, and this is one reason a screenshot of it would have been misleading even if someone had |
...
| **Verify `GET /admin/campaign-design` response shape** | Low — largely settled | Downgraded from High. Sessions 81 and 85 both shipped block types *through* this endpoint (`hero`, then `group_list`), and in each case backend stated the served shape concretely and it matched what `lib/campaign-design.ts` normalises — including the session-85 fix that flattened `item_fields` into the same shape as a block's own `fields`. That is two independent confirmations. **Still not literally checked against a live response**, so this stays open rather than closed, but the "editor unavailable, use Write HTML" fallback firing after deploy is now an unlikely outcome rather than a plausible one. Original: The one unverified assumption in the campaign builder — no live response was available from this side, so `lib/campaign-design.ts` normalises several plausible key spellings rather than guessing one. If the block editor comes up on the "not available on this server yet" fallback after deploy, **this is the first thing to check**. Also unverified: block-instance field names (`{ type, ...fields }`, inferred from the example payload) and the `test-send` recipient key (`email` assumed) |
