...
> **Not built (in scope terms, flagged not forgotten):** the `emails` selector on `move-market` is a
> paste-a-list path with no UI yet — the endpoint accepts it, and it's the natural next step for a
> marketer holding a list of addresses rather than a checkbox selection. Small addition if wanted.
